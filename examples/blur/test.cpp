...
#pragma omp parallel for
        for (int yTile = 0; yTile < out.height(); yTile += 32) {    // loop n strip of 32 scanlines each tile
            __m128i tmp[(128 / 8) * (32 + 2)];
            for (int xTile = 0; xTile < out.width(); xTile += 128) {    // loop n strip of 128 columns each tile

            // calc each tile 
                __m128i *tmpPtr = tmp;
                // blur_x
                for (int y = 0; y < 32 + 2; y++) {  // loop each scanline of a tile
                    const uint16_t *inPtr = &(in(xTile, yTile + y));
                    for (int x = 0; x < 128; x += 8) {      // loop each block of a tile
...
int main(int argc, char **argv) {
    ...
    printf("Image %dx%d process time: %f %f %f\n", width, height, slow_time, fast_time, halide_time);
